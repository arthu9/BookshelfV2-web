        ...
        if request.method == 'POST':
            ...
            response = requests.post(
                ...
                json={"current_user": session['user'], 'title': request.form['title'], "edition": request.form['edition'],
                      "year": request.form['year'], "author_name": request.form['author_name'],
                      "description": request.form['description'], "price": price, "price_rate": priceRate,
            ...
            return render_template('addbook.html', display='none')
    else:
        return redirect('unauthorized')
...
def add_borrow(bookshelf_id, book_id, username):
    if g.user:
        ...
        if request.method == 'POST':
            ...
            url = 'http://localhost:5050/bookshelf/borrow_book'
            response= requests.post(url,headers={'x-access-token': session['token']}, json={"bookshelf_id": bookshelf_id, "book_owner": username, "book_borrower": session['user'], "book_id": book_id, "start": date_start, "end": date_end})
            book_dict=json.loads(response.text)
            print(response.text)
            url2 = 'http://localhost:5050/bookshelf/borrow/user'
            books = requests.get(url2, headers={'x-access-token': session['token']}, json={"current_user": session['user']})
            book_dict2 = json.loads(books.text)
            if book_dict['message'] == "You can't add your own book to your borrow":
                if not book_dict2['book']:
                    message = "No books to display"
                    return render_template('borrow.html', books=book_dict2['book'], message=book_dict['message'],
                                           color='danger', nodisplay=message)
                return render_template('borrow.html', books=book_dict2['book'], message=book_dict['message'], color='danger')
            elif book_dict['message'] == '"Book is already in rental':
                return render_template('borrow.html', books=book_dict2['book'], message=book_dict['message'], color='danger')
            elif book_dict['message'] == 'Failed to add':
                if not book_dict2['book']:
                    message = "No books to display"
                ...
                print(books.text)
                return render_template('borrow.html', books=book_dict2['book'])
    else:
        return redirect('unauthorized')
...
def borrow():
    if g.user:
        url = 'http://localhost:5050/bookshelf/borrow/user'
        books = requests.get(url, headers={'x-access-token': session['token']}, json={"current_user": session['user']})
        book_dict = json.loads(books.text)
        print(books.text)
        # print(book_dict['book']['start'])
        # start1 = book_dict['book']['start']

        # start2 = parse(start1)
        print(book_dict['book'])
        if not book_dict['book']:
            message="No books to display"
            return render_template('borrow.html', books={}, nodisplay=message)
        return render_template('borrow.html', books=book_dict['book'])
    else:
        return redirect('unauthorized')
...
def remove_borrow(username, book_id):
    if g.user:
        url= 'http://localhost:5050/bookshelf/remove_borrow'
        response = requests.post(url, headers={'x-access-token': session['token']}, json={"book_id": book_id, "bookshelf_owner": username, "username": session['user']})
        book_dict = json.loads(response.text)
        return redirect(url_for('borrow',book=book_dict['message']))
    else:
        return redirect('unauthorized')
...
def rental():
    if g.user:
        url = 'http://localhost:5050/bookshelf/rental/user'
        books = requests.get(url, headers={'x-access-token': session['token']}, json={"current_user": session['user']})
        book_dict = json.loads(books.text)
        print(books.text)
        # print(book_dict['book']['start'])
        ...
        if not book_dict['book']:
            message="No books to display"
            return render_template('rental.html', books={}, nodisplay=message)
        return render_template('rental.html', books=book_dict['book'])
    else:
        return redirect('unauthorized')


@app.route('/bookshelf/rental/<username>/<bookshelf_id>/<book_id>', methods=['GET','POST'])
def add_rental(bookshelf_id, book_id, username):
    if g.user:
        print(bookshelf_id)
        print(book_id)
        print(username)
        if request.method == 'POST':
            date_start = request.form['start']
            date_end = request.form['end']
            url = 'http://localhost:5050/bookshelf/rent_book'
            response= requests.post(url, headers={'x-access-token': session['token']}, json={"bookshelf_id": bookshelf_id, "book_owner": username, "book_borrower": session['user'], "book_id": book_id, "start": date_start, "end": date_end})
            book_dict=json.loads(response.text)
            print(response.text)
            url2 = 'http://localhost:5050/bookshelf/rental/user'
            books = requests.get(url2, headers={'x-access-token': session['token']}, json={"current_user": session['user']})
            book_dict2 = json.loads(books.text)
            if book_dict['message'] == "You can't add your own book to your rental":
                if not book_dict2['book']:
                    message = "No books to display"
                    return render_template('rental.html', books=book_dict2['book'], message=book_dict['message'],
                                           color='danger', nodisplay=message)
                return render_template('rental.html', books=book_dict2['book'], message=book_dict['message'], color='danger')
            elif book_dict['message'] == '"Book is already in rental':
                return render_template('rental.html', books=book_dict2['book'], message=book_dict['message'], color='danger')
            elif book_dict['message'] == 'Failed to add':
                if not book_dict2['book']:
                ...
                print(books.text)
                return render_template('rental.html', books=book_dict2['book'])
    else:
        return redirect('unauthorized')

@app.route('/bookshelf/rental/remove/<username>/<book_id>', methods=['GET', 'POST'])
def remove_rental(username, book_id):
    if g.user:
        url= 'http://localhost:5050/bookshelf/remove_rental'
        response = requests.post(url, headers={'x-access-token': session['token']}, json={"book_id": book_id, "bookshelf_owner": username, "username": session['user']})
        book_dict = json.loads(response.text)
        return redirect(url_for('rental',book=book_dict['message']))
    else:
        return redirect('unauthorized')
...
def purchase():
    if g.user:
        url = 'http://localhost:5050/bookshelf/purchase/user'
        books = requests.get(url,  headers={'x-access-token': session['token']}, json={"current_user": session['user']})
        book_dict = json.loads(books.text)
        print(books.text)
        # print(book_dict['book']['start'])
        ...
        if not book_dict['book']:
            message="No books to display"
            return render_template('purchase.html', books={}, nodisplay=message)
        return render_template('purchase.html', books=book_dict['book'])
    else:
        return redirect('unauthorized')
...
def add_purchase(bookshelf_id, book_id, username):
    if g.user:
        ...
        print(username)
        if request.method == 'POST':
            url = 'http://localhost:5050/bookshelf/purchase_book'
            response= requests.post(url,  headers={'x-access-token': session['token']}, json={"bookshelf_id": bookshelf_id, "book_owner": username, "book_borrower": session['user'], "book_id": book_id})
            book_dict=json.loads(response.text)
            print(response.text)
            url2 = 'http://localhost:5050/bookshelf/purchase/user'
            books = requests.get(url2,  headers={'x-access-token': session['token']}, json={"current_user": session['user']})
            book_dict2 = json.loads(books.text)
            if book_dict['message'] == "You can't add your own book to your rental":
                if not book_dict2['book']:
                    message = "No books to display"
                    return render_template('purchase.html', books=book_dict2['book'], message=book_dict['message'],
                                           color='danger', nodisplay=message)
                return render_template('purchase.html', books=book_dict2['book'], message=book_dict['message'], color='danger')
            elif book_dict['message'] == '"Book is already in rental':
                return render_template('purchase.html', books=book_dict2['book'], message=book_dict['message'], color='danger')
            elif book_dict['message'] == 'Failed to add':
                if not book_dict2['book']:
                ...
                print(books.text)
                return render_template('purchase.html', books=book_dict2['book'])
    else:
        return redirect('unauthorized')

@app.route('/bookshelf/purchase/remove/<username>/<book_id>', methods=['GET', 'POST'])
def remove_purchase(username, book_id):
    if g.user:
        url= 'http://localhost:5050/bookshelf/remove_purchase'
        response = requests.post(url,  headers={'x-access-token': session['token']}, json={"book_id": book_id, "bookshelf_owner": username, "username": session['user']})
        book_dict = json.loads(response.text)
        return redirect(url_for('purchase',book=book_dict['message']))
    else:
        return redirect('unauthorized')
