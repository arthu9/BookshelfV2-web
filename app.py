...
def api_login(username, password, response):
    ...
    url = 'http://localhost:5555/user/info/'+username
...
def home():
    if g.user:
        url = 'http://localhost:5555/bookshelf/books/latest'
    ...
    if g.user:
        if request.method == 'POST':
            ...
            if len(interests) == 1:
                ...
                url = 'http://localhost:5555/interests/'+interests
                requests.post(url, json={"current_user": session['user']})
            else:
                for interest in interests:
                    print(interest)
                    url='http://localhost:5555/interests/'+interest
...
def addbook():
    if g.user:
        if request.method == 'POST':
            ...
            requests.post(
                'http://localhost:5555/user/addbook',
...
def profile():
    if g.user:
        book_details = requests.get('http://localhost:5555/user/bookshelf/availability', json={"current_user": session['user']})
        books = requests.get('http://localhost:5555/user/bookshelf', json={"current_user": session['user']})
        if books.text == "no books found":
            return render_template('no_books_profile.html')
        book_dict=json.loads(books.text)
...
def viewbook(book_id, username):
    if g.user:
        book = requests.get('http://localhost:5555/user/bookshelf/'+book_id, json={"current_user": username})
...
def view_genre(genre_name):
    if g.user:
        books = requests.get('http://localhost:5555/interests/view/'+genre_name)
        print(books.text)
        book_dict = json.loads(books.text)
...
def store():
    if g.user:
        books = requests.get('http://localhost:5555/bookshelf/books')
        book_dict = json.loads(books.text)
        if not book_dict['book']:
...
def add_wishlist(bookshelf_id, book_id):
    if g.user:
        if request.method == 'POST':
            url = 'http://localhost:5555/bookshelf/wishlist'
            ...
            print(response.text)
            url2 = 'http://localhost:5555/bookshelf/wishlist/user'
            ...
            if book_dict['message'] == "You can't add your own book to your wishlist":
                ...
                print(book_dict['message'])
                return render_template('wishlist.html', books=book_dict2['book'], message=book_dict['message'], color='success')
    else:
        return redirect('unauthorized')
...
def wishlist():
    if g.user:
        url = 'http://localhost:5555/bookshelf/wishlist/user'
...
#
#     url = 'http://localhost:5555/user/info/<username>'
#     r = requests.get(url=url, headers={'x-access-token':  })
...
def search():
    if request.method == "POST":
            ...
            r = requests.get('http://localhost:5555/search', json={'item': request.form['info']})
            print(r.text)
